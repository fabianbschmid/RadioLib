...
class LR2021: public LRxxxx {
  public:
    ...
    */
    ...
    /*!
      \brief Initialization method for the Bluetooth LE physical layer.
      \details This method initializes the BLE modem with the specified configuration.
      Only the physical layer is provided by the chip: preamble, access address, whitening
      and the 24-bit CRC. The PDU header is not generated by the transceiver and has to be
      built by the host and passed as the leading bytes of the payload. There is no link
      layer, so this cannot form BLE connections and does not do link layer encryption.
      Supports designated initializers when using C++14 or above.
      \param config Initialization configuration.
      \returns \ref status_codes
    */
    int16_t beginBLE(const LR2021BleConfig_t& config);

    /*!
      \brief Initialization method for the Bluetooth LE physical layer.
      \param freq Carrier frequency in MHz. Defaults to 2402.0 MHz (advertising channel 37).
      \param phy BLE PHY, one of RADIOLIB_LR2021_BLE_PHY_*. Defaults to LE 1M.
      \param channel BLE channel index (0 - 39), used to derive the whitening initial value.
      Defaults to 37, which matches the default frequency.
      \param power Output power in dBm. Defaults to 10 dBm.
      \param tcxoVoltage TCXO reference voltage to be set. Defaults to 1.6 V.
      If you are seeing -706/-707 error codes, it likely means you are using non-0 value for module with XTAL.
      To use XTAL, either set this value to 0, or set LR2021::XTAL to true.
      \returns \ref status_codes
    */
    int16_t beginBLE(float freq = 2402.0, uint8_t phy = RADIOLIB_LR2021_BLE_PHY_1M, uint8_t channel = 37, int8_t power = 10, float tcxoVoltage = 1.6);

    /*!
      \brief Derive the BLE whitening initial value from a channel index.
      \param channel BLE channel index (0 - 39). Channels 37, 38 and 39 are the primary
      advertising channels.
      \returns The value to pass as LR2021BleConfig_t::whiteningInit.
    */
    static uint8_t bleWhiteningInit(uint8_t channel);
  ...
  private:
    ...
    // cached BLE PHY, needed by the time-on-air math - the packet structure and the
    // symbol rate both depend on it, and neither can be read back from the chip
    uint8_t blePhy = RADIOLIB_LR2021_BLE_PHY_1M;

    // cached BLE Rx FIFO layout, so getTimeOnAir and readData agree on whether the
    // 3 CRC bytes are part of the buffer
    bool bleCrcInFifo = false;
    ...
    // BLE commands
    int16_t setBleModulationParams(uint8_t phy, uint8_t rxBw = RADIOLIB_LR2021_BLE_RX_BW_AUTO);
    int16_t setBleChannelParams(uint8_t channelType, uint8_t whiteningInit, uint32_t crcInit, uint32_t syncWord, bool crcInFifo);
    int16_t setBleTxPduLen(uint8_t len);
    int16_t setBleTx(uint8_t len);
    int16_t getBleRxStats(uint16_t* packetRx, uint16_t* crcError, uint16_t* lenError);
    int16_t getBlePacketStatus(uint16_t* payloadLen, float* rssiAvg, float* rssiSync, float* lqi);

    // BLE errata workarounds, ported from the Semtech LR20xx driver (lr20xx_workarounds.c).
    // These are applied automatically by beginBLE and setBleModulationParams.
    int16_t bleFixCodedSyncWord();
    int16_t bleFixCodedFreqDrift();
    int16_t bleFixPreamble2M();
